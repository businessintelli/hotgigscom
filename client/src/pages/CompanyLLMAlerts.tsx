...
import { AlertCircle, Bell, Plus, Trash2, CheckCircle2, XCircle, Info } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

type AlertType = "usage_threshold" | "cost_threshold" | "error_rate";
type Period = "hourly" | "daily" | "weekly" | "monthly";

export default function CompanyLLMAlerts() {
  const { user } = useAuth();
  ...
  const handleCreateAlert = () => {
    if (!threshold || !emailRecipients || !companyId) {
  ...
  return (
    <CompanyAdminLayout>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">AI Usage Alerts</h1>
          <p className="text-muted-foreground mt-1">
            Get notified when your company's AI usage exceeds thresholds
          </p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Set up automated notifications for your company's AI usage
              </DialogDescription>
            </DialogHeader>
      ...
      <Alert>
        <Info className="w-4 h-4" />
        <AlertDescription>
          Alerts help you proactively manage AI costs and avoid unexpected charges. 
          Set conservative thresholds initially and adjust based on your usage patterns.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Your Company's Alerts</CardTitle>
          <CardDescription>
            Configured alert rules for monitoring AI usage
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loadingAlerts ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            History of alerts triggered for your company
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loadingHistory ? (
              ...
              {alertHistory.map((history: any) => (
                <div key={history.history.id} className="flex items-start justify-between p-4 border rounded-lg">
                  ...
                  <Badge variant={history.history.emailSent ? "default" : "destructive"}>
                    {history.history.emailSent ? "Notified" : "Failed"}
                  </Badge>
                </div>
              ))}
            </div>
