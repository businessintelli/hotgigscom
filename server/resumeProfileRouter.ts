...
export const resumeProfileRouter = router({
  ...
  // Save resume profile after review/edit
  saveResumeProfile: protectedProcedure
    .input(z.object({
      candidateId: z.number(),
      profileName: z.string(),
      resumeUrl: z.string(),
      fileKey: z.string(),
      fileName: z.string(),
      parsedData: z.any(), // Edited ParsedResume object
      isDefault: z.boolean(),
    }))
    .mutation(async ({ input }) => {
      const { candidateId, profileName, resumeUrl, fileKey, fileName, parsedData, isDefault } = input;
      
      // Calculate scores from edited parsed data
      let scores = null;
      try {
        scores = calculateResumeScores(parsedData);
        scores.topDomains = calculateTopDomains(parsedData);
      ...
      // Create resume profile with edited data
      ...
      // Also update candidate profile with parsed data
      await db.updateCandidate(candidateId, {
        ...
        skillCategories: parsedData.metadata?.skillCategories ? JSON.stringify(parsedData.metadata.skillCategories) : undefined,
